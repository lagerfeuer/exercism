defmodule ComplexNumbers do
  import Kernel, except: [div: 2]

  @typedoc """
  In this module, complex numbers are represented as a tuple-pair containing the real and
  imaginary parts.
  For example, the real number `1` is `{1, 0}`, the imaginary number `i` is `{0, 1}` and
  the complex number `4+3i` is `{4, 3}'.
  """
  @type complex :: {float, float}

  defguardp is_complex(x)
            when is_tuple(x) and tuple_size(x) == 2 and is_number(elem(x, 0)) and
                   is_number(elem(x, 1))

  defp complex(c) when is_complex(c), do: c
  defp complex(r) when is_number(r), do: {r, 0}

  @doc """
  Return the real part of a complex number
  """
  @spec real(a :: complex) :: float
  def real({r, _i}), do: r

  @doc """
  Return the imaginary part of a complex number
  """
  @spec imaginary(a :: complex) :: float
  def imaginary({_r, i}), do: i

  @doc """
  Multiply two complex numbers, or a real and a complex number
  """
  @spec mul(a :: complex | float, b :: complex | float) :: complex
  def mul(a, b) do
    with {r1, i1} <- complex(a),
         {r2, i2} <- complex(b) do
      {r1 * r2 - i1 * i2, r1 * i2 + r2 * i1}
    end
  end

  @doc """
  Add two complex numbers, or a real and a complex number
  """
  @spec add(a :: complex | float, b :: complex | float) :: complex
  def add(a, b) do
    with {r1, i1} <- complex(a),
         {r2, i2} <- complex(b) do
      {r1 + r2, i1 + i2}
    end
  end

  @doc """
  Subtract two complex numbers, or a real and a complex number
  """
  @spec sub(a :: complex | float, b :: complex | float) :: complex
  def sub(a, b) do
    with {r1, i1} <- complex(a),
         {r2, i2} <- complex(b) do
      {r1 - r2, i1 - i2}
    end
  end

  @doc """
  Divide two complex numbers, or a real and a complex number
  """
  @spec div(a :: complex | float, b :: complex | float) :: complex
  def div(a, b) do
    with {r1, i1} <- complex(a),
         {r2, i2} <- complex(b) do
      {(r1 * r2 + i1 * i2) / (r2 * r2 + i2 * i2), (r2 * i1 - r1 * i2) / (r2 * r2 + i2 * i2)}
    end
  end

  @doc """
  Absolute value of a complex number
  """
  @spec abs(a :: complex) :: float
  def abs({r, i}), do: :math.sqrt(r * r + i * i)

  @doc """
  Conjugate of a complex number
  """
  @spec conjugate(a :: complex) :: complex
  def conjugate({r, i}), do: {r, -i}

  @doc """
  Exponential of a complex number
  """
  @spec exp(a :: complex) :: complex
  def exp({r, i}), do: {:math.exp(r) * :math.cos(i), :math.exp(r) * :math.sin(i)}
end
