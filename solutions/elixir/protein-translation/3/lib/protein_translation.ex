defmodule ProteinTranslation do
  ...
  def of_rna(rna), do: decode(rna)

  defp decode(rna, decoded \\ [])

  defp decode(<<codon::binary-size(3), rna::binary>>, decoded) do
    case of_codon(codon) do
      {:ok, "STOP"} -> decode("", decoded)
      {:ok, protein} -> decode(rna, [protein | decoded])
      {:error, _} -> {:error, "invalid RNA"}
    end
  end

  defp decode("", decoded), do: {:ok, Enum.reverse(decoded)}
  defp decode(_, _), do: {:error, "invalid RNA"}
